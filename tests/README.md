...
uv run pytest tests/ --cov=mcp_ahrefs --cov-report=term --cov-report=html
...
uv run pytest tests/ --cov=mcp_ahrefs --cov-report=html
...
import pytest
from mcp_ahrefs.tools.my_tools import my_new_tool
...
uv run pytest tests/ --cov=mcp_ahrefs --cov-fail-under=40

# Generate coverage report for CI artifacts
uv run pytest tests/ --cov=mcp_ahrefs --cov-report=xml
```
...
4. Check coverage hasn't decreased: `uv run pytest tests/ --cov=mcp_ahrefs`
