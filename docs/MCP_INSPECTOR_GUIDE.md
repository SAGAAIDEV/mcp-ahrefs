# MCP Inspector Testing Guide for MCP Ahrefs
...
which mcp
# Should show: /path/to/mcp_ahrefs/.venv/bin/mcp
...
1. Use the full path to your venv's mcp:
   ```bash
   .venv/bin/mcp dev mcp_ahrefs/server/app.py
   ```
...
```bash
mcp dev mcp_ahrefs/server/app.py
```
...
**Text Logs**:
- macOS: `~/Library/Logs/mcp-servers/mcp_ahrefs.log`
- Linux: `~/.local/state/mcp-servers/logs/mcp_ahrefs.log`
- Windows: `%LOCALAPPDATA%\mcp-servers\logs\mcp_ahrefs.log`

**SQLite Database** (tool execution history):
- macOS: `~/Library/Application Support/mcp_ahrefs/tool_logs.db`
- Linux: `~/.local/share/mcp_ahrefs/tool_logs.db`
- Windows: `%LOCALAPPDATA%\mcp_ahrefs\tool_logs.db`
...
```bash
streamlit run mcp_ahrefs/ui/app.py
```
...
If you see `ModuleNotFoundError: No module named 'mcp_ahrefs'`:

1. Ensure you're in the project root directory
2. Check virtual environment is activated: `which python`
3. Reinstall: `uv sync`
4. Use full mcp path: `.venv/bin/mcp dev mcp_ahrefs/server/app.py`

### MCP Inspector Not Loading

1. Check server started without errors
2. Try a different port: `mcp dev mcp_ahrefs/server/app.py --port 5174`
...
1. Add your own tools in `mcp_ahrefs/tools/`
