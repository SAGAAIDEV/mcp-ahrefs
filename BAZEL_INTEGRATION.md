...
2. **Python**: Ensure Python 3.12 is available
...
bazel build //mcp_ahrefs:mcp_ahrefs
```
...
```bash
# Run the MCP server
bazel run //mcp_ahrefs:mcp_ahrefs

# Run with arguments
bazel run //mcp_ahrefs:mcp_ahrefs -- --help
```
...
- **py_binary**: `mcp_ahrefs` - The executable MCP server
- **py_library**: `mcp_ahrefs_lib` - The library containing all source files
