...
def get_sqlite_logs(correlation_id: str) -> List[Dict]:
    ...
    import platformdirs
    app_data = platformdirs.user_data_dir("mcp_ahrefs")
...
async def test_all_tools(server_script_path: str):
    ...
    server_params = StdioServerParameters(
        command="python",
        args=["-m", "mcp_ahrefs.server.app", "--transport", "stdio"],
        env=env
    )
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            # Define test cases for all 6 tools
            test_cases = [
                {
                    "tool": "echo_tool",
            ...
            console.print("1. Run the Streamlit UI: [cyan]streamlit run mcp_ahrefs/ui/app.py[/cyan]")
            ...
            db_path = Path.home() / ".local" / "share" / "mcp_ahrefs" / "unified_logs.db"
...
def main(server_module: str):
    ...
    2. Calls all 6 example tools with unique correlation IDs
