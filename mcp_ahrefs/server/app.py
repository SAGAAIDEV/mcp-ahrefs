"""MCP Ahrefs - MCP Server with SAAGA Decorators
...
from mcp_ahrefs.config import ServerConfig, get_config
from mcp_ahrefs.logging_config import setup_logging, logger
from mcp_ahrefs.logging.correlation import (
    generate_correlation_id,
    set_initialization_correlation_id,
    clear_initialization_correlation_id
)
from mcp_ahrefs.logging.unified_logger import UnifiedLogger
...
def create_mcp_server(config: Optional[ServerConfig] = None) -> FastMCP:
    ...
    from mcp_ahrefs.logging.destinations import DestinationConfig
    ...
    import logging
    unified_logger = logging.getLogger('mcp_ahrefs')
    ...
    mcp_server = FastMCP(config.name or "MCP Ahrefs")
...
def main(port: int, transport: str) -> int:
    """Run the MCP Ahrefs server with specified transport."""
