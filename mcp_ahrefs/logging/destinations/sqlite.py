...
class SQLiteDestination(LogDestination):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        """Get logging statistics from the database.
        ...
        # Total log count
        cursor = conn.execute("SELECT COUNT(*) as count FROM unified_logs")
        ...
        cursor = conn.execute("""
            ...
            GROUP BY log_type
        """)
        stats['logs_by_type'] = {row['log_type']: row['count'] for row in cursor}
        ...
        cursor = conn.execute("""
            ...
            GROUP BY level
        """)
        stats['logs_by_level'] = {row['level']: row['count'] for row in cursor}
        ...
        cursor = conn.execute("""
            ...
            FROM unified_logs
            WHERE log_type = 'tool_execution' AND status IS NOT NULL
        """)
        row = cursor.fetchone()
